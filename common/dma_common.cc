...
int counter = 0;
int my_send_queue_length = 0;
int my_recv_queue_length = 0;

DMAcontext* DMA_create(ibv_device* ib_dev, int queue_length, int thread_id)
{
    my_send_queue_length = 2048;
    my_recv_queue_length = my_send_queue_length * 8;
    ...
    int send_buf_size = P4ML_PACKET_SIZE * my_recv_queue_length;
    ...
    uint8_t* mp_send_ring = (uint8_t*)malloc(tx_ring_size);
    ...
    uint8_t* mp_recv_ring = (uint8_t*)malloc(kAppRingSize);
    ...
    install_flow_rule(mp_recv_qp, thread_id);
    ...
    fprintf(stderr, "kAppRingMbufSize=%lu, kAppStridesPerWQE=%lu, kAppRingSize=%lu, kAppRQDepth=%lu\n", kAppRingMbufSize, kAppStridesPerWQE, kAppRingSize, kAppRQDepth);
    ...
    printf("[Thread %d] Finish created QP with length: %d\n", thread_id, queue_length);
...
void send_packet(DMAcontext* dma_context, int chunk_size, uint64_t offset)
{
    ...
    memcpy(hdr, IP_ETH_UDP_HEADER, IP_ETH_UDP_HEADER_SIZE); // Assuming that the header buffer was define before.
	...
	// //     if (ntohs(p4ml_header->seq_num) == 32000)   {
	//         // if (p4ml_header->flag & 2 && ntohs(p4ml_header->seq_num) != 32000) {
    //     p4ml_header_print_h((agghdr*)((char *)sg.addr + i * P4ML_LAYER_SIZE), "SEND");
	// //             printf("p4ml_header->seq_num: %d\n", ntohs(p4ml_header->seq_num));
	// //             // exit(1);
	// 		//  }
	//     }
	// // }
    ...
    if (dma_context->isMarkTimeStamp) {
        ...
        for (int i = 0; i < chunk_size / P4ML_LAYER_SIZE; i++) {
            ...
            if (!dma_context->isSent[ntohs(p4ml_header->seq_num)]) {
                ...
                dma_context->send_time[ntohs(p4ml_header->seq_num)] = current_time;
            } else {
...
// Install a flow rule
void install_flow_rule(struct ibv_qp* qp, uint16_t thread_id)
{
    ...
    unsigned char R_DST_MAC[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05 };
